...
- 站内定邀候选页 `/targeted-screening`：按 TikTok Shop 定邀口径（1,000–100,000 粉、成交 ≥100 件、平均播放 ≥100）预览粘贴表格，按 TikTok 账号跨项目查重，记录近 30 天邀约、黑名单、内容与受众核验，显示逐人排除或待补证据原因；仅同一 30 天统计周期的候选参与相对排序。录入和核验不会发送邀请；站内发送仍需人工完成。
...
1. 在 `/targeted-screening` 粘贴人工整理的制表符表格、CSV 或 Markdown 表，预览后确认录入。建议保留“来源截图”编号；应用不假设后台能导出 CSV。
2. 逐人核对主页、同一近 30 天数据、近期内容、受众、历史邀请、黑名单和竞品关系，并填写有证据的个性化切入点。只有资料齐全的候选可加入审核包。
